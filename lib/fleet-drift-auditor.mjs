...
// A renamed repository keeps answering to its old path: GitHub redirects, every fetch succeeds,
// and nothing reports it. That is not hypothetical here — `fleet.upstream.repository` said
// `minipuft/claude-prompts` for months after the rename to `minipuft/claude-prompts-mcp`, every
// audit run resolved it through the redirect, and no run mentioned it. The value was corrected by
// hand; nothing detected it, and nothing would have detected it coming back.
//
// The redirect is a live supply-chain exposure, not untidiness. An abandoned name stays claimable
// by anyone, and the day it is claimed every consumer silently retargets to a stranger's files —
// the identical reasoning `claude-prompts-mcp` uses to assert its marketplace source URL.
//
// So a mismatch is DRIFT. The API answers a request for a renamed repository with the CANONICAL
// `full_name`, which makes the detection exact rather than heuristic: declared !== canonical means
// the declared path is a redirect. An unresolved probe is a NOTE, matching auditCheckOutcomes —
// an unverified rule announces itself rather than passing silently and restoring the blind spot.
export function auditCanonicalName(declared, canonical, violations, notes) {
  if (canonical === undefined) {
    notes.push(
      `Canonical name for ${declared} was not resolved; rename-redirect exposure is unverified`,
    );
    return;
  }
  if (canonical !== declared) {
    violations.push(
      `repository resolves through a rename redirect: declared ${declared}, canonical ${canonical}`,
    );
  }
}
...
export function auditFleet(fleet, snapshots) {
  ...
  auditCanonicalName(
    ...
    snapshots.upstreamCanonicalName,
    upstream.violations,
    upstream.notes,
  );
  return {
